...
public class GluCalcSQLiteHelper extends SQLiteOpenHelper {
	...
	public void store(List<Food> foods) {
		...
		try {
			...
			for (Food food : foods) {
				...
				values.put(FoodTable.COLUMN_CARBONHYDRATE,
						food.getCarbonhydrate());

				long id = db.insert(FoodTable.TABLE_NAME, "", values);
				food.setId(id);
				values.clear();
			}
	...
	public List<Food> loadFoods() {
		Cursor cursor = getReadableDatabase().query(FoodTable.TABLE_NAME,
				FoodTable.COLUMNS, null, null, null, null, null);
		List<Food> foods = new ArrayList<Food>(cursor.getCount());
